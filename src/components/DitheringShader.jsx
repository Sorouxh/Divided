...
void main() {
  ...
  vec2 swirlUv = uv / max(u_zoom, .001);
  float swirlRadius = length(swirlUv);
  float t = .5 * u_time;
  vec2 waveUv = uv * 4.0;
  ...
  float angle = 6.0 * atan(swirlUv.y, swirlUv.x) + 2.0 * u_time;
  float twist = fract(pow(max(swirlRadius, .001), -1.2) + angle / TWO_PI);
  float swirl = mix(0.0, twist, smoothstep(0.0, 1.0, pow(swirlRadius, 1.2)));
